<?php
require('require/top.php');
authorise_user();
user_vfd_efd($con);
$checkout = array();
$checkout = get_chekout_products($con);
$subtotal = 0;
?>
<div class="path">
    <div class="container">
        <a href="index.php">الرئيسية</a>
        /
        <a href="checkout.php">الدفع</a>
    </div>
</div>
<section class="checkout">
    <section class="myac-body">
        <div class="flex row">
            <div class="right">
                <div class="col-lg-12 col-md-12">
                    <div class="pdpt-bg">
                        <form action="process.php" method="POST" id="checkout-form">
                            <div class="step">
                                <div class="tittle" onclick="open_address(this)" id="ct-ad">
                                    <span>1</span>
                                    <h4>عنوان التسليم</h4>
                                </div>
                                <div class="span" id="chekout_address" style="height: 0">
                                    <div class="formbody">
                                        <div class="form-group">
                                            <div class="product-radio">
                                                <ul class="product-now">
                                                    <li>
                                                        <input type="radio" id="ad1" name="address1" checked="" value="البيت" />
                                                        <label for="ad1">البيت</label>
                                                    </li>
                                                    <li>
                                                        <input type="radio" id="ad2" name="address1" value="المكتب" />
                                                        <label for="ad2">مكتب</label>
                                                    </li>
                                                    <li>
                                                        <input type="radio" id="ad3" name="address1" value="اخرى" />
                                                        <label for="ad3">اخرى</label>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="address-fieldset">
                                            <div class="row">
                                                <div class="row2">
                                                    <div class="lt">
                                                        <label for="ft">الاسم *</label>
                                                        <input type="text" placeholder="الاسم" id="dv-name" />
                                                    </div>
                                                    <div class="ft">
                                                        <label for="ft"> الجوال *</label>
                                                        <input type="number" placeholder="رقم" id="dv-number" oninput="validate_number()" />
                                                    </div>
                                                </div>
                                                <label for="ft">مدينة*</label>
                                                <select name="" id="dv-city">
                                                    <option value="#">اختر المدينة </option>
                                                    <?php
                                                    $querys = "select * from city order by id desc";
                                                    $ress = mysqli_query($con, $querys);
                                                    while ($rows = mysqli_fetch_assoc($ress)) {
                                                    ?>
                                                        <option value="<?php echo $rows['id'] ?>">
                                                            <?php echo $rows['city_name'] ?></option>
                                                    <?php
                                                    }
                                                    ?>
                                                </select>
                                                <label for="ft">رقم الشقة / المنزل /المكتب *</label>
                                                <input type="text" placeholder="عموان" id="dv-address" />
                                                <div class="row2">
                                                    <div class="lt">
                                                        <label for="ft">الرمز البريدي *</label>
                                                        <input type="text" placeholder="الرمز البريدي" id="dv-pin" />
                                                    </div>
                                                    <div class="ft">
                                                        <label for="ft">شارع*</label>
                                                        <input type="text" placeholder="شارع" id="dv-land" />
                                                    </div>
                                                </div>
                                                <div class="row2">
                                                    <a href="javascript:void(0)" onclick="add_new_address()" class="save-address">حفظ</a>
                                                    <a href="javascript:void(0)" class="next-step" onclick="nex_ad()">التالي</a>
                                                </div>
                                                <br>
                                                <h4>العنوان المضاف</h4>
                                                <div class="row2 mt2" style="display:block" id="ad-ad">
                                                    <?php

                                                    $template = '';
                                                    $uid = $_SESSION['USER_ID'];
                                                    $res = mysqli_query($con, "select user_address.*,city.city_name from user_address,city where user_address.uid='$uid' and user_address.user_city=city.id");
                                                    while ($row = mysqli_fetch_assoc($res)) {
                                                        $template = $template . '
                                                                <div class="address-item">
                                                                <input type="radio" name="dv-ad" value="' . $row['id'] . '"
                                                                    style="width:2rem; height:1.5rem;margin-right:0.8rem;margin-top:0;">
                                                                <div class="address-icon1">
                                                                    <i class="uil uil-home"></i>
                                                                </div>
                                                                <div class="address-dt-all">
                                                                    <h4>' . $row['type_ad'] . '</h4>
                                                                    <p>
                                                                        ' . $row['user_name'] . ', ' . $row['user_local'] . ', ' . $row['user_add'] . ',
                                                                        ' . $row['city_name'] . ', ' . $row['user_pin'] . ',<br>' . $row['user_mobile'] . '
                                                                    </p>
                                                                </div>
                                                                </div>
                                                            ';
                                                    }
                                                    echo $template;
                                                    ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="step">
                                <div class="tittle" onclick="open_dvt(this)" id="ct-dt">
                                    <span>2</span>
                                    <h4>موعد وتاريخ التسليم</h4>
                                </div>
                                <div class="span" id="chekout_dvt" style="height: 0">
                                    <div class="formbody">
                                        <div class="form-group">
                                            <div class="product-radio">
                                                <ul class="product-now">
                                                    <li>
                                                        <input type="radio" id="ad5" name="address3" checked="" value="ايوم" />
                                                        <label for="ad5">اليوم</label>
                                                    </li>
                                                    <li>
                                                        <input type="radio" id="ad4" name="address3" value="غداً" />
                                                        <label for="ad4">غداً</label>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="time-radio">
                                            <div class="form">
                                                <div class="fields">
                                                    <?php
                                                    $res = mysqli_query($con, "select * from dv_time");
                                                    while ($r = mysqli_fetch_assoc($res)) {
                                                    ?>
                                                        <div class="field">
                                                            <div class="ui radio checkbox chck-rdio">
                                                                <input type="radio" name="fruit" tabindex="0" class="hidden" value="<?php echo $r['id'] ?>" />
                                                                <label><?php echo $r['from'] ?> - <?php echo $r['tto'] ?></label>
                                                            </div>
                                                        </div>
                                                    <?php } ?>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row2">
                                        <a href="javascript:void(0)" class="next-step" onclick="nex_pt()">Proceed To
                                            Pay</a>
                                    </div>
                                </div>
                            </div>
                            <div class="step">
                                <div class="tittle" onclick="open_pt(this)" id="ct-pt">
                                    <span>3</span>
                                    <h4>الدفع</h4>
                                </div>
                                <div class="span" id="chekout_pt" style="height: 0">
                                    <div class="formbody">
                                        <div class="rpt100">
                                            <ul class="radio--group-inline-container_1" style="flex-direction:column;display:flex;">
                                                <li style="width:100%;height:6rem;">
                                                    <div class="radio-item_1">
                                                        <input id="cashondelivery1" value="1" name="paymentmethod" type="radio" />
                                                        <label for="cashondelivery1" class="radio-label_1">الدفع عند الاستلام</label>
                                                    </div>
                                                </li>
                                            <!--    <li style="width:100%; height:6rem;">
                                                    <div class="radio-item_1">
                                                        <input id="card1" value="2" name="paymentmethod" type="radio" />
                                                        <label for="card1" class="radio-label_1">Credit / Debit
                                                            Card</label>
                                                    </div>
                                                </li>-->
                                            </ul>
                                        </div>
                                    </div>
                                    <input type="hidden" name="order-id" id="order-id">
                                    <div class="row2">
                                        <button class="next-step">وضع الطلب</button>
                                    </div>
                                </div>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
            <div class="left">
                <div class="ul-wrapper">
                    <h4>ملخص الطلب</h4>
                    <?php foreach ($checkout as $product) { ?>
                        <div class="cart-item border_radius">
                            <div class="cart-product-img">
                                <img src="media/product/<?php echo $product['img1']; ?>" alt="" />
                                <div class="offer-badge">
                                    <?php
                                    $offn = ($product['fa'] * 100) / $product['price'];
                                    $off = round(100 - $offn);
                                    echo $off . '%';
                                    ?>
                                </div>
                            </div>
                            <div class="cart-text">
                                <h4><?php echo $product['product_name']; ?></h4>
                                <div class="cart-item-price"> ريال <?php echo $product['fa'];
                                                                    $subtotal += $product['fa'] * $product['qty'];
                                                                    ?>
                                    <span> ريال <?php echo $product['price']; ?></span>
                                </div>
                                <br>
                                <div class="cart-item-price">
                                    <?php echo "x" . $product['qty']; ?>
                                </div>
                            </div>
                        </div>
                    <?php } ?>
                    <div class="total-checkout-group">
                        <div class="cart-total-dil">
                            <h4>المجموع الفرعي</h4>
                            <span>ريال <?php echo $product['total']; ?></span>
                        </div>
                        <div class="cart-total-dil pt-3">
                            <h4>رسوم التوصيل</h4>
                            <span> ريال <?php
                                            echo $product['ship_fee'];
                                            ?></span>
                        </div>
                        <?php
                        if ($product['is_applied']) {
                        ?>
                            <div class="cart-total-dil pt-3">
                                <h4>العرض الترويجي</h4>
                                <span>-  ريال <?php
                                                echo $product['promo'];
                                                ?></span>
                            </div>
                        <?php } ?>
                        <?php
                        if ($product['is_add_w']) {
                        ?>
                            <div class="cart-total-dil pt-3">
                                <h4>من المحفظلة</h4>
                                <span>-  ريال <?php
                                                echo $product['wl_amt'];
                                                ?></span>
                            </div>
                        <?php } ?>
                    </div>
                    <div class="main-total-cart">
                        <h2>المجموع</h2>
                        <span> ريال <?php echo $product['final_amt']; ?></span>
                    </div>
                    <div class="cktout">
                        <i class="uil uil-padlock"></i> الدفع الامن
                    </div>
                </div>
                <div class="promocode">
                    <a href="javascript:void(0)" onclick="show_promo()">لديك رمز ترويجي؟</a>
                </div>
                <div class="promoform" id="promoform">
                    <form action="javascript:void(0)">
                        <?php
                        if ($product['is_applied'] == 0) {
                        ?>
                            <input type="text" placeholder="ادخل الرمز الترويجي" id="promocode" />
                            <button onclick="apply_promo()">ادخال</button>
                        <?php } else {
                        ?>
                            <button onclick="remove_promo()" style="width:100%;border-radius:0;">حذف الرمز الترويجي</button>
                        <?php
                        } ?>
                    </form>
                </div>
                <div class="promocode">
                    <a href="javascript:void(0)" onclick="show_wt()">استخدام محفظتي</a>
                </div>
                <div class="promoform" id="wt">
                    <form action="javascript:void(0)">
                        <?php
                        if ($product['is_add_w'] == 0) {
                        ?>
                            <button onclick="apply_wallet()" style="width:100%;border-radius:0;">استخدام</button>
                        <?php } else {
                        ?>
                            <button onclick="remove_wallet()" style="width:100%;border-radius:0;">حذف</button>
                        <?php
                        } ?>

                    </form>
                </div>
            </div>
        </div>
    </section>
</section>
<script src="assets/js/ckt.js"></script>
<?php require('require/foot.php'); ?>
<?php require('require/last.php'); ?>
